...
2. correlate: `mcp__github__list_pull_requests` (closed/merged, newest first) +
   `mcp__github__list_commits` on the repo. A change merged shortly before the
   spike is the prime suspect; confirm with `mcp__github__get_pull_request_files`.
3. search: `mcp__github__search_issues` for "payments" / "checkout 5xx" — link if found.
...
5. report: `mcp__github__add_issue_comment` — findings (impact, suspect PR + link, evidence) on the incident issue.
